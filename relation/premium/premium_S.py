...
def premium_S():
    ...
    for s in x:
        ...
        gks = quote_option(op)
        premium.append(gks.theoretical)
        intrinsic_value.append( op.pay_off(Decimal(s)) )
        time_value.append( gks.theoretical - op.pay_off(Decimal(s)) )

    line_plot(x,[premium, intrinsic_value, time_value],
              legend = ['premium', 'intrinsic_value', 'time_value'],
